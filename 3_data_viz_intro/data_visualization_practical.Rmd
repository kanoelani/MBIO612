...
```{r}
# Write your code here
ggplot(data = mpg) + geom_point(color = 'blue', mapping = aes(x = displ, y = hwy)) + facet_grid(class ~ drv)
...
ggplot(data = mpg, aes(x = cty, y = hwy, color = class)) + 
  geom_point(size = 6) + geom_smooth(method = lm, color = "red")
```
...
```{r}
# Write your code here
subcompact_15_cty <- mpg[mpg$class == "subcompact" & mpg$cty == 15, c("manufacturer", "model")]
subcompact_15_cty
```
...
```{r}
 ggplot(data = sizeMass, aes(x = Size, y = Mass)) +
      geom_point() +
...
ggplot(data = sizeMass, aes(x = size_log10, y = mass_log10)) + 
