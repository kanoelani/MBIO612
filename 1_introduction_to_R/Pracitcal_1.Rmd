...
the vector c(1, 2, 3, "Hi") contains both numeric values (1, 2, 3) and a character value ("Hi")

* Would the following be valid?

`c(1, 2, 3, "Hi") + 1 `

It would not be valid because you cannot perform arithmetic operations directly on a vector that contains a mixture of numeric and non-numeric (character) elements.

* How about 

`c(1, 2, 3, FALSE) + 1 `

It would be valid as it contains numeric values (1, 2, 3) and a logical value (FALSE). When you attempt to add 1 to the entire vector, R will coerce the logical value to numeric (where FALSE becomes 0 and TRUE becomes 1) and then perform the addition element-wise.
...
library(ggplot2)
# Write you code in this cell
qplot(x_axis, y_axis, geom = "point", xlab = "My x_axis", ylab = "My y_axis", main = "My amazing plot of x_axis versus y_axis")
